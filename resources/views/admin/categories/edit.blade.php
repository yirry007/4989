...
@section('content')
<div class="pd-20">
  <form action="<?php echo e(url('/admin/categories/'.$field->id)); ?>" method="post" class="form form-horizontal">
      ...
      <div class="row cl">
          ...
          <div class="formControls col-6"> <span class="select-box">
			<select class="select" size="1" name="parent_id">
				<option value="0" <?php if($field->parent_id == 0) echo 'selected'; ?> >Top Cate</option>
                <?php foreach($categoryData as $v): ?>
                <?php if(in_array($v->id, $categoryChildren) || $v->id == $field->id) continue; ?>
                <option value="<?php echo e($v->id); ?>" <?php if($field->parent_id == $v->id) echo 'selected'; ?> ><?php echo e(str_repeat('▶', 2*$v->level).$v->name); ?></option>
                <?php endforeach; ?>
			</select>
        </span> </div>
          <div class="col-3"> </div>
