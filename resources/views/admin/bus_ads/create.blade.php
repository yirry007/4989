...
<div class="pd-20">
  <form action="<?php echo e(url('/admin/bus_ads')); ?>" method="post" class="form form-horizontal" id="form-member-add">
  	<input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>" />
      <div class="row cl">
          <label class="form-label col-3">Partners：</label>
          <div class="formControls col-6"> <span class="select-box">
            <select class="select" size="1" name="partner_code">
                <?php foreach($partners as $v): ?>
                <option value="<?php echo e($v->code) ?>" <?php if(session('input')['partner_code'] == $v->code) echo 'selected'; ?>><?php echo e($v->name) ?></option>
                <?php endforeach; ?>
            </select>
        </span> </div>
      </div>
      ...
      <div class="row cl" style="margin: 0;">
          ...
          <div class="formControls col-6" style="color: #f8658f;">
              <span style="color: #999">Best upload image size</span> W : H(* : *)
          </div>
      </div>
	  <div class="row cl">
...
<script>
    $('#file_upload').uploadify({
        ...
        'uploader' : "<?php echo e(url('/admin/upload_one/bus_ads')) ?>",
