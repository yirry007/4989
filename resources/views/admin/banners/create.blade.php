...
<div class="pd-20">
  <form action="<?php echo e(url('/admin/banners')); ?>" method="post" class="form form-horizontal" id="form-member-add">
  	<input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>" />
      <div class="row cl">
          <label class="form-label col-3">Position：</label>
          <div class="formControls col-6 skin-minimal">
              <div class="radio-box">
                  <input type="radio" name="position" value="main" <?php if(session('input')['position'] === 'main' || session('input')['position'] === NULL) echo 'checked'; ?>>
                  <label>Main</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="list" <?php if(session('input')['position'] === 'list') echo 'checked'; ?>>
                  <label>List</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="ads" <?php if(session('input')['position'] === 'ads') echo 'checked'; ?>>
                  <label>Ads</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="main_mid" <?php if(session('input')['position'] === 'main_mid') echo 'checked'; ?>>
                  <label>Main Mid</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="main_popup" <?php if(session('input')['position'] === 'main_popup') echo 'checked'; ?>>
                  <label>Main Popup</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="sign" <?php if(session('input')['position'] === 'sign') echo 'checked'; ?>>
                  <label>Sign Page</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="mypage_ad" <?php if(session('input')['position'] === 'mypage_ad') echo 'checked'; ?>>
                  <label>My page</label>
              </div>
              <div class="radio-box">
                  <input type="radio" name="position" value="fiveday_off" <?php if(session('input')['position'] === 'fiveday_off') echo 'checked'; ?>>
    ...
    <div class="row cl">
      ...
      <div class="row cl">
          ...
          <div class="formControls col-6">
              <input type="text" class="input-text" value="<?php echo e(session('input')['link']); ?>" placeholder="ex：https://www.baidu.com/" name="link" >
          </div>
      </div>
      <div class="row cl">
          <label class="form-label col-3">End Time：</label>
          <div class="formControls col-2">
              <input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" id="datemax" class="input-text Wdate" style="width:180px;" name="end_time" value="<?php echo e(session('input')['end_time']); ?>">
          </div>
      </div>
      <div class="row cl">
